...
We will create graph by reading  all the nodes and edges from edgelist using in-built function nx.read_edgelist().
We have to find the length of shortest path(no. of edges between the shortest path) between each pair of nodes. For that we will create a list 'N' containing all nodes of 
graph and wll also create an empty list 'SPLL' for storing the length. Then, run a loop to find length l of shortest path between each pair of node and append this l to SPLL for each 
pair of node.
Find minimum, maximum and average value of SPLL. We will come to know that average value of SPLL is 6 i.e. on avg. length of shortest path between any pair of node is 6
